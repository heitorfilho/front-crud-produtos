...
export class FornecedorService extends BaseService {
    ...
    obterPorId(id: string): Observable<Fornecedor> {
        return this.http
            .get<Fornecedor>(this.UrlServiceV1 + "fornecedores/" + id, this.ObterHeaderJson())
            .pipe(catchError(super.serviceError));
    }
    ...
    atualizarFornecedor(fornecedor: Fornecedor, id: string): Observable<Fornecedor> {
        return this.http
            .put(this.UrlServiceV1 + "fornecedores/" + id, fornecedor, this.ObterHeaderJson())
            .pipe(
                map(super.extractData),
                catchError(super.serviceError)
            );
